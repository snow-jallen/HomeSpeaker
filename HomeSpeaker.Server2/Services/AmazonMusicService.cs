...
/// <summary>
/// Integrates with the <c>amazon-music</c> Python package (<c>pip install amazon-music</c>) to list
/// configured playlists and download their tracks for local playback.
/// <para>
/// Setup:
/// 1. Install the CLI: <c>pip install amazon-music</c><br/>
...
public class AmazonMusicService
{
    ...
    /// <summary>Returns true when the <c>amz</c> CLI is available on the host's PATH.</summary>
    ...
    {
        var playlist = GetConfiguredPlaylists().FirstOrDefault(p => p.Id == playlistId);
        if (playlist is null)
        ...
        this.logger.LogInformation("Downloading playlist '{Name}' at quality {Quality}", playlist.Name, quality);
        ...
        try
        {
            ...
            // Pass arguments individually to avoid token appearing in a single logged string
            startInfo.ArgumentList.Add(playlist.Url);
            startInfo.ArgumentList.Add("-q");
            startInfo.ArgumentList.Add(quality);
            startInfo.ArgumentList.Add("-o");
            startInfo.ArgumentList.Add(downloadDir);
        ...
        catch (Exception ex) when (ex is InvalidOperationException or System.ComponentModel.Win32Exception)
        {
            ...
            return (false, "The 'amz' CLI was not found. Install it with: pip install amazon-music");
        }
        ...
        if (downloadedSongs.Count == 0)
        {
            ...
            this.logger.LogInformation("Playing {Count} tracks from filesystem (library sync may be in progress)", audioFiles.Count);
            this.player.Stop();
            ...
            return (true, $"Now playing {audioFiles.Count} tracks from '{playlist.Name}' (shuffled).");
        }
        ...
        this.logger.LogInformation("Enqueued {Count} shuffled tracks from Amazon playlist '{Name}'",
